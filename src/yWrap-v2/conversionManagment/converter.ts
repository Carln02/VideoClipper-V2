...
import {YPrimitive, YSharedType, YValue, YValueNew, YWrappedValue} from "./conversionManagement.types";
...
const yCache: WeakMap<YSharedType, YWrappedObject> = new WeakMap();
...
export function yjsToWrapped(yValue: YValueNew, ownProperty: boolean = true): YWrappedValue {
...
export function yMapToWrapped(yMap: YMap): YWrappedObject {
    ...
    if (yCache.has(yMap)) return yCache.get(yMap);
