...
export class TimelineModel extends YArrayManagerModel<SyncedClip, Clip> {
    ...
    private readonly pathModel: YArrayManagerModel<SyncedFlowPath, object>;
    ...
    private _clipListsData: YArray<SyncedClip>[] = [];
    ...
    @auto()
    public set clipListsData(value: YArray<SyncedClip>[]) {
        this.clear();
        this.path.length = 0;
        value.forEach((entry, index) => this.setClipList(entry, index));
    }

    public setClipList(data: YArray<SyncedClip>, index: number = -1) {
        if (this.path.length == 0) index = 0;
        else if (index < 0) index = this.path.length - 1;
        this.path.splice(index, 0, );
        this.clipListsData[index] = data;
    }
